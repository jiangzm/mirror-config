...
function envInProfile (env) {
	// 环境变量信息转换为shell脚本
	const sh = getMirrors(env, (key, value) => value && `export ${key}=${value}`).sort();

	sh.unshift('\n\n# Created by mirror-config');
	sh.push(
		// 'export NVM_DIR="$HOME/.nvm"',
		// '[ -s "$NVM_DIR/nvm.sh" ] && \\. "$NVM_DIR/nvm.sh"  # This loads nvm',
		// '[ -s "$NVM_DIR/bash_completion" ] && \\. "$NVM_DIR/bash_completion"  # This loads nvm bash_completion',
		'# End of mirror-config'
	);

	const shFilePath = '/etc/profile.d/node.sh';
	...
	return fs.readFile(shFilePath, 'utf8').catch(() => '').then(content => {
		...
		if (newContent !== content.trim()) {
			sudo.writeFile(shFilePath, newContent + '\n');
		}
	});
}
